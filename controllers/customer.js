...
router.get(['/', '/home'], async function (req, res) {
  ...
  var hotproducts = await ProductDAO.selectTopHot(5);
...
router.get('/listproduct', async function (req, res) {
  ...
  res.render('../views/customer/shop.ejs', { cats: categories, newprods: products });
});
router.get('/search', async function (req, res) {
  var categories = await CategoryDAO.selectAll();
  var keyword = req.query.txtKeyword;
...
router.get('/verify', async function (req, res) {
  ...
  if (result) {
    MyUtil.showAlertAndRedirect(res, 'OK !', './login');
  } else {
    MyUtil.showAlertAndRedirect(res, 'SORRY !', './signup');
  }
});
...
router.post('/login', async function (req, res) {
  ...
  if (customer && customer.active == 1) {
    ...
    MyUtil.showAlertAndRedirect(res, 'SORRY !', './login');
  }
});
...
router.post('/myprofile', async function (req, res) {
  ...
  if (curCust) {
    ...
    if (result) {
      ...
      MyUtil.showAlertAndRedirect(res, 'OK !', './home');
    }
  }
  MyUtil.showAlertAndRedirect(res, 'SORRY !', './myprofile');
});
// myorders
...
router.get('/checkout', async function (req, res) {
  if (req.session.customer) {
    ...
    if (result) {
      ...
      MyUtil.showAlertAndRedirect(res, 'SORRY !', './mycart');
    }
  } else {
    res.redirect('./login');
  }
...
router.get(['/', '/shop'], async function (req, res) {
  ...
  res.render('../views/customer/shop.ejs', { cats: categories, newprods: newproducts });
});
//render about
